    ...
    >
      <AnimatePresence mode='wait' >
        {enabled && (
          <motion.div
            key={`2`}
            className="relative w-4/5 max-w-[240px] p-1 mb-4 rounded-full shadow-lg pointer-events-auto bg-primary-50/40 dark:bg-primary-700/60 backdrop-blur-2xl backdrop-filter sm:w-3/5 shadow-primary-700/10 overflow-hidden"
            ...
            <div className="flex items-center space-x-4">
              <div className="flex-shrink-0">
                {image ? (
                  ...
                  <Icon className="w-5 h-5 ml-2 text-primary-800 dark:text-primary-200" />
                )}
              </div>
              <div className="flex-grow">
                <h4 className="text-sm text-center text-primary-800 dark:text-primary-200">
                  {title ? title : t(`Clicked`)}
                </h4>
                <p className="text-xs font-semibold text-center text-primary-600 dark:text-primary-400">
                  {message}
                </p>
              </div>
              <button
                className="p-1.5 transition rounded-full mr-2 hover:bg-primary-400/10 dark:hover:bg-primary-200/10 flex-shrink-0 relative right-1"
                onClick={() => onDismiss()}
              >
                <span className="sr-only">{t("Close Notification")}</span>
                <XIcon className="w-4 h-4 text-primary-800 dark:text-primary-200" />
              </button>
            </div>
          </motion.div>
        )}
