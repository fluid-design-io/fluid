...
function ListNestedAnimateComponent({ setNotification, ...props }) {
  ...
  return (
    ...
    >
      <nav
        className="flex-1 p-2 prefers-contrast:divide-y prefers-contrast:divide-stone-600 dark:prefers-contrast:divide-stone-200"
        aria-label="Sidebar"
      >
        {navigation.map(({ children, name, Icon }) =>
          !children ? (
            <button
              key={`nav.${name}`}
              className={`flex items-center justify-start w-full px-4 py-2 capitalize transition outline-none select-none rounded-md ${rowStyle}`}
              onClick={() =>
                setNotification({
                  enabled: true,
